...
def bann_text():
 ...
 | |  | |/ _` | '__| |/ / . ` |/ _ \| __/ __| '_ \ 
 | |__| | (_| | |  |   <| |\  | (_) | || (__| | | |
 |_____/ \__,_|_|  |_|\_\_| \_|\___/ \__\___|_| |_|
...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("DarkBomb was updated to the latest version")
        mesgdcrt.GeneralMessage("Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update DarkBomb.")
        mesgdcrt.WarningMessage("Grab The Latest one From https://github.com/vipuluthaiah/DarkNotch.git")
...
def do_git_update():
    ...
    if success:
        ...
        print("git checkout . && git pull https://github.com/vipuluthaiah/DarkNotch.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get("https://raw.githubusercontent.com/TheSpeedX/DarkBomb/master/.version").text.strip()
    if fver != __VERSION__:
        mesgdcrt.WarningMessage("An update is available")
        mesgdcrt.GeneralMessage("Starting update...")
        update()
    else:
        mesgdcrt.SuccessMessage("DarkBomb is up-to-date")
        mesgdcrt.GeneralMessage("Starting DarkBomb")

def notifyen():
    try:
        noti = requests.get("https://raw.githubusercontent.com/TheSpeedX/DarkBomb/master/.notify").text.upper().strip()
        if len(noti) > 10:
...
def pretty_print(cc,target,success,failed):
    ...
    mesgdcrt.SuccessMessage("DarkBomb was created by SpeedX")
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit={"sms":1000,"call":100,"mail":2000}
        cc,target="",""
        if mode in ["sms","call"]:
            cc,target=get_phone_info()
            if cc!="900":
                max_limit.update({"sms":900})
        elif mode=="mail":
...
if sys.version_info[0]!=3:
    mesgdcrt.FailureMessage("DarkBomb will work only in Python v3")
    sys.exit()
...
description="""DarkBomb - Your Friendly Spammer Application

DarkBomb can be used for many purposes which incudes - 
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

DarkBomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,epilog='Coded by SpeedX !!!')
parser.add_argument("-sms","--sms", action="store_true",help="start DarkBomb with SMS Bomb mode")
parser.add_argument("-call","--call", action="store_true",help="start DarkBomb with CALL Bomb mode")
parser.add_argument("-mail","--mail", action="store_true",help="start DarkBomb with MAIL Bomb mode")
parser.add_argument("-u","--update", action="store_true",help="update DarkBomb")
parser.add_argument("-c","--contributors", action="store_true",help="show current DarkBomb contributors")
parser.add_argument("-v","--version", action="store_true",help="show current DarkBomb version")
